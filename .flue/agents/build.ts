...
const npm = defineCommand('pnpm');
const gh = defineCommand('gh', { env: { GH_TOKEN: process.env.GH_TOKEN } });
const curl = defineCommand('curl');
const playwrightCli = defineCommand('playwright-cli');

const CLAUDE_OPUS_4_7 = 'github-copilot/claude-opus-4.7';
const OPENAI_GPT_5_5 = 'github-copilot/gpt-5.5';

export default async function ({env, init}: FlueContext) {
	...
	const agent = await init({
		...
		commands: [npm, gh, curl, playwrightCli],
		tools:[
			...github.tools,
		]
	});
	...
	const technicalResearcher = await agent.session("Technical Researcher", { role: 'researcher' });
	const technicalImplementer = await agent.session("Technical Implementer", { role: 'swe' });
	...
	while (qualityAssuranceChecks['prosemirror-research'] === false) {
		...
		const result = await technicalResearcher.prompt(researchPrompt, {
			result: v.object({
				filesChanged: v.array(v.string()),
			}),
		});

		console.log('[Result]: Files Changed:\n', result.filesChanged.join(',\n'));

		const qa = await agent.session("Quality Assurance", { role: 'quality assurance' });

		console.log('[Task]: Quality Assurance Check for prosemirror research');

		const qaResults = await qa.prompt(
			`
			<original request>
		...
		const qaText = qaResults.text as string;
	...
	while (qualityAssuranceChecks['implementation'] === false) {
		...
		const result = await technicalImplementer.prompt(implementPrompt, {
			result: v.object({
				summary: v.string(),
			}),
			model: OPENAI_GPT_5_5
