...
- [x] Drag-to-reorder actually works. Previously the `dragover` handler bailed when `dataTransfer.types` did not advertise `application/x-plim-block`, but Chrome strips access to custom types on same-document drags. The view now tracks an `activeDragSourceId` flag set on a `plim:dragstart` custom event from the handle and consumed by `onDrop`, falling back to `dataTransfer.getData(...)` for cross-document drops.
